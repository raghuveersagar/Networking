/*
 * This class acts as the client of the two player game.
 * @author Raghuveer Sagar
 * @author Himanshu Mendhe
 */
...
public class LinkImageClient implements Runnable {

	int buttonWidth = 100;
	int buttonHeight = 100;
	int imageSetSelected = 1;
	JLabel labelP1;
	JLabel labelP2;
	...
	public void run() {
		...
		try {
			Socket echoSocket = new Socket("localhost", 16000);
			...
			while ((userInput = in.readLine()) != null) {
				...
				RemoteInfo obj = (RemoteInfo)r.lookup("serverObj");	
				 userInput = obj.getInfo();
				alienClick = true;
	...
	public static void main(String[] args) {
		...
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				LinkImageClient l = new LinkImageClient();
				l.go();
	...
	void go() {
		...
		frame = new JFrame("Find the linking image: Player 2");
	...
	public class ImageSetPanel extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			if (index <= numberOfSets) {
				Color c = g.getColor();
System.out.println("imageSetSelected"+imageSetSelected);
